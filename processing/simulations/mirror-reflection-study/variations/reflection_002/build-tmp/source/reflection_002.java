...
public class reflection_002 extends PApplet {

 //line intersection lib schon bei java dabei. hurra
 //line intersection lib schon bei java dabei. hurra
...
public void draw() {
    ...
    if (MODE == _MOUSE) {
        r = fixAngle(PI - ((PI / 2.0f) + calcRotationAngleInDegrees(new PVector(mouseX , mouseY ), beamOrigin)));
    } else if (MODE == _ROT) {
        if (!PAUSE) { r = r + 0.01f; }
    }
...
public void compReflection(PVector origin, double direction, int depth) {
...
public PVector oldsegIntersection(float x1, float y1, float x2, float y2, float x3, float y3, float x4, float y4) {
    double bx = x2 - x1;
    double by = y2 - y1;
...
public double calcRotationAngleInDegrees(PVector centerPt, PVector targetPt) {
    double theta = Math.atan2(targetPt.y - centerPt.y, targetPt.x - centerPt.x);
    theta = ((Math.PI / 2.0f) - theta) + PI ;
    return theta;
}
...
public void keyReleased() {
  ...
  static public void main(String[] passedArgs) {
    String[] appletArgs = new String[] { "reflection_002" };
    if (passedArgs != null) {
