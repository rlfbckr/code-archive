...
public class mirrorReflectionsExample extends PApplet {
...
public void draw() {
    ...
    if (MODE == _MOUSE) {
        beam.setBeamRotation(  PI - ( (PI / 2.0f) + beam.calcRotationAngle(new PVector(mouseX , mouseY ), beam.getBeamOrigin())));
        //beam2.setBeamRotation(  PI - ( (PI / 2.0) + beam2.calcRotationAngle(new PVector(mouseX , mouseY ), beam2.getBeamOrigin())));
    } else if (MODE == _ROT) {
        if (!PAUSE) {
            beam.setBeamRotation(beam.getBeamRotation() + 0.01f);
...
class Beam {
    ...
    public void compReflection(PVector origin, float direction, int depth, Mirror currentMirror) {
        ...
        if (inter != null) {
            ...
            // draw beam
            stroke(map(depth, 0, MAX_REFLECTION_DEPTH, 255, 10), 0, 0); // fade out beam color...
...
class Mirror  {
  ...
  static public void main(String[] passedArgs) {
    String[] appletArgs = new String[] { "mirrorReflectionsExample" };
    if (passedArgs != null) {
